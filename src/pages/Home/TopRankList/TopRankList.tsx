...
function TopRankList() {
  ...
  return (
    <s.SectionContainer>
      ...
      <ts.GenreFilter>
        <ts.GenreButton selected={selectedGenre === ''} onClick={() => handleGenreChange('')}>
          전체
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'GGGA'} onClick={() => handleGenreChange('GGGA')}>
          뮤지컬
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'AAAA'} onClick={() => handleGenreChange('AAAA')}>
          연극
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'CCCD'} onClick={() => handleGenreChange('CCCD')}>
          대중음악
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'CCCA'} onClick={() => handleGenreChange('CCCA')}>
          서양음악(클래식)
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'CCCC'} onClick={() => handleGenreChange('CCCC')}>
          한국음악(국악)
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'EEEB'} onClick={() => handleGenreChange('EEEB')}>
          서커스/마술
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'BBBC'} onClick={() => handleGenreChange('BBBC')}>
          무용(서양/한국무용)
        </ts.GenreButton>
        <ts.GenreButton selected={selectedGenre === 'BBBE'} onClick={() => handleGenreChange('BBBE')}>
          대중무용
        </ts.GenreButton>
      </ts.GenreFilter>
